...
Hyper Difference Frequency Generation (HDFG) spectroscopy is an underdeveloped four wave mixing vibrational spectroscopy based on the hyper-Raman transition. 
Unlike other coherent four-wave mixing techniques, HDFG only relies on coupling between a vibrational and electronic state.
...
HDFG is found to possess selection rules based upon infrared absorption spectroscopy and hyper-Raman scattering, making it sensitive to vibronic coupling mechanisms in isotropic systems. 
Applying the selection rules found for vibronic hyper-Raman scattering, it is found that HDFG output is dependent upon potential well offset.
It is found that HDFG and vibrational sum frequency generation (vSFG) spectroscopy share similar output, making HDFG feasible for vSFG practitioners. 
\end{abstract}
...
The three-wave mixing ($\chi^{(2)}$) techniques are often less applicable as they require macroscopic non-centrosymmetry under the electric dipole approximation.\cite{RN133}
...
When the hyper-Raman excitation is resonant or near-resonant with electronic states, the brightness of the vibrational feature depends on the vibronic structure of the electronic state.
The electronic spectrum probed in HDFG can inform on processes that control ultrafast electronic relaxation in molecular and biological systems in similar ways to resonance Raman.\cite{Bredenbeck2015, Arsenault2021}
...
We note that HDFG emission can be phase-matched in media with normal dispersion, while HSFG cannot,\cite{RN278} although experimental strategies can be implemented to minimize phase-mismatch in HSFG processes. \cite{Kaufman2024}  
...
\subsection{Electronically resonant and pre-resonant: Vibronic Selection Rules}\label{Albrecht}
...
To employ this formalism, we write the states in a Born-Oppenheimer basis $\ket{a,b}$, where $\ket{a,b} = |a(Q)) \otimes \ket{b}$ for electronic states $\{|a(Q))\}$ and vibrational states $\{\ket{b}\}$ (i.e., adiabatic approximation), where $Q$ is the adiabatic parameter. \cite{BornOppenheimer, Tang1970}
...
Here, we restrict $v' = \{0,1,2\}$ for simplicity.

The impact of vibronic coupling in the HDFG spectrum is made clear by using a simple model. \cite{Kundu2022}
...
We stress that while the $A,B$ selection rules are based upon any hyper-Raman active mode, the $\mel{v}{\mu_l}{0}$ term in \autoref{gammaabc} imposes the constraint that the $A,B$ transitions occur for infrared active normal mode.
...
Note that in the limit of complete potential energy surface overlap ($\xi \rightarrow 0$), $A$ vanishes as $\lim_{\xi \rightarrow 0} \langle 1 | 0 \rangle = 0$ for harmonic potential wells.\cite{Myers1982}
This selection rule is imposed by the dipole transition in HDFG. 
$B$ does not vanish in the same limit, as $\lim_{\xi \rightarrow 0} \mel{0}{Q}{1} = 1/\sqrt{2}$ for wells with identical curvature.
...
The lack of $\ket{v'}$ dependence in the resonance denominator causes $A$ to vanish through closure.
However, the simulated spectrum (\autoref{fig:doubres_spec}b) shows a non-negligible $A$ contribution for $\omega_2 < 20000$ cm$^{-1}$, the limit for this model system where $\abs{\omega_{ev',g0} - 2\omega_2} \gg 2\Gamma_{ev',g0}$.
...
This discussion assumes that mode symmetry allows for coupling between all of the vibronic states considered here.
...
\begin{table}[!htbp]
	...
	\begin{ruledtabular}
		\begin{tabular}{cccc}
			Resonance Term & $\ket{e,0}$ & $\ket{e,1}$ & $\ket{e,2}$\\
			\hline  
			$A$ 
...
\section{Appendix: Comparison of vSFG and two-beam HDFG output}\label{appendixA}
 ...
 This method provides practitioners of sum frequency generation the ability to measure spectra and dephasing dynamics in isotropic media without many meaningful changes in their optical setup, and thus compare spectroscopy and dephasing in the bulk and at the interface.\cite{RN224}
