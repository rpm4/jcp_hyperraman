...
It is often sufficient to write $\dot{\rho}_R = \Gamma \rho$, i.e. exponential relaxation (Markovian dynamics), where $\Gamma$ is the system's pure dephasing rate. \cite{RN455, RN307, Yurs2012}
In the exponential relaxation limit, the solution for a density matrix element $\rho_{ab}$, assuming only one input field, is written as
\begin{equation}
	\begin{split}
		\rho_{ab} &= \text{stuff} \int_0^\infty \mathrm{d}t \exp({-it}(\omega_{ab} - \omega - i\Gamma_{ab})) \\
		&= \frac{\text{stuff}}{\omega_{ab} - \omega - i\Gamma_{ab}}\\
	\end{split}
\end{equation}
This limit is often appropriate for isolated molecules at low temperatures. \cite{RN287}
However, exponential relaxation is an inappropriate description of most physical systems when system-bath interactions are included. \cite{Sue1986, Yan1988, Li1994, RN287, Myers1997, Sung2001, RN445}
Such systems are properly described by including Brownian bath oscillators, which changes density matrix propagation as
\begin{equation}
	\text{evil stuff}
\end{equation}
Nevertheless, in the discussion that follows, we assume the limit of an isolated molecule with no bath interaction and that pulse widths are longer than the electronic dephasing times, which has proven useful in investigating the properties of doubly resonant SFG spectroscopy, 2D-EV spectroscopy and 2D-VE spectroscopy. \cite{Raschke2002, Gaynor2017} 
This limit is taken to emphasize the quantum mechanical selection rules of the HDFG technique. 
We stress that in most systems, e.g., solvated chromophores, system-bath interactions must be explicitly included to properly describe lineshapes of vibronic phenomena. \cite{Li1994}
Future work will more rigorously investigate the impact of system-bath interactions on HDFG spectral analysis.\cite{McDonnellinprep}
...
In $B_1$, the Herzberg-Teller coupling is associated with the two-photon absorption process, whereas in $B_2$, the Herzberg-Teller coupling is associated with one-photon emission.\footnote{Franck-Condon factors and Herzberg-Teller integrals are most easily evaluated in the position basis.}
...
\subsection{Linenarrowing Inhomogeneous Lineshapes}
...
\end{equation} 
While Gaussian distributions are commonly used to describe inhomogeneous broadening,\cite{RN307} Lorentzian distributions have successfully described inhomogeneous broadening in a variety of spectroscopies and yield  closed-form solutions. \cite{Dick83_1, Carlson1990line}
Such approaches have been suitable for investigating the inhomogeneity of excitonic resonances in CdSe quantum dots through FWM spectroscopy. \cite{Yurs2011, Yurs2012}
