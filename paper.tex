...
All spectra are reported on the intensity scale. %@dan if we should change to amp lmk
...
In previous studies, following the method introduced by Levenson and Bloembergen, an internal standard was added to the sample.
The internal standard in most experiments was chosen to be C$_6$H$_6$ or C$_6$D$_6$, due to the strong Raman activity and measured $\chi^{(3)}$ of its ring breathing mode ($\nu_2$ in Herzberg notation). \cite{Levenson1974_2, RN351, RN345}
Interference between the sample and internal standard resonances allowed measurment of $\chi^{(3)}$.
Such a method does not require measurment of absolute intensities and instead relies on self-heterodyning of the internal standard signal. 
Resonant SIVE lineshapes are also complicated by amplitude level interference between the sample and its substrate and/or sample cell windows. \cite{RN362}
...
\begin{split}
		\chi^{(3)}_{IJKL} &= NF(\omega_4) \langle \gamma_{ijkl} \rangle \sim \langle \beta_{ijk} \mu_l \rangle\\
\end{split}
\end{equation}
This expression cannot be exactly used to extract $\beta_{ijk}$, as $\beta_{ijk}$ is a Hermitian operator but $\Delta_{gv} \in \mathbb{C}$. 
However, when on resonance, $\Delta_{gv} = -i\Gamma_{gv}$.
Thus, below we consider cases where the experiment is resonant, so that $\chi^{(3)}_{IJKL}$ is purely imaginary, or, (insert equation).
Clearly, we can then see that $\langle \beta_{ijk} \mu_l \rangle \sim -\frac{1}{\Gamma_{gv}}\Im(\chi^{(3)}_{IJKL})$ 
...
\begin{subequations}
	\begin{equation}
		\begin{split}
			\langle \gamma_{ijkl} \rangle &\sim \langle \beta_{ijk} \mu_l \rangle\\
...
Continuing by substituting in the results found in \autoref{SIVEselection}, we see
	\begin{equation}
		\langle \beta_{ijk} \rangle = - 24\sqrt{3} \frac{\hbar \varepsilon_0 \Delta_{gv}}{NF(\omega_4)\rho_{gg}} \frac{\chi^{(3)}_{IJKL}}{\mu}
	\end{equation}
\end{subequations}
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=3.375in]{wmel2.png}
...
\begin{figure*}[!htbp]
	\centering
	\includegraphics[width=6.66in]{cob.png}
	\caption{
