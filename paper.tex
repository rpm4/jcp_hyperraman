...
\section{Density Matrix Propagation}
In the cases discussed in the introduction, the proposed HDFG spectroscopy method is used to probe vibronic coupling.
Coherent spectroscopies are often described using the density-matrix formalism, where a density matrix $\rho$ is propagated using the Liouville-von Neumann equation as
\begin{equation}
	\dot{\rho} = \frac{1}{i \hbar} [H_0 + H(t), \rho] + \dot{\rho}_R,
\end{equation}
where $H_0$ is system Hamiltonian, $H(t)$ is the time-dependent, matter-radiation interaction Hamiltonian and $\rho_R$ is the relaxation density matrix. 
In most treatments of coherent vibrational spectroscopy, it is sufficient to write $\dot{\rho}_R = \Gamma \rho$, i.e. exponential relaxation (Markovian dynamics), where $\Gamma$ is the system's pure dephasing rate. \cite{RN455, RN307}
...
It is useful to expose relationships between nonlinear output, transition dipoles and hyper-Raman hyperpolarizabilities in the driven limit. \cite{Simpson2004, Moad2005}
...
\begin{table}[!htbp]
	...
	Closed-form expressions for the Franck-Condon and Herzberg-Teller integrals used here have been tabulated by Carlson.\cite{Carlson1988thesis}}\footnote{The Herzberg-Teller Integrals 
	\begin{ruledtabular}
		\begin{tabular}{cccc}
...
vSFG is a $\chi^{(2)}$ technique whose output, When only vibrationally resonant, the vSFG $\chi^{(2)}$ scales as $\chi^{(2)}_{IJK} \sim \langle \alpha_{ij} \mu_k \rangle$, where $\alpha_{ij}$ is the Raman polarizability tensor, whose phase-matching constraint is  $\vec{k}_3 = \vec{k}_1 + \vec{k}_2$.
