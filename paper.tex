...
%todo: discussion on ABC terms and what they mean.
...
While Placzek approximation type arguments are useful to understand the general source of HDFG output, it is insightful to interpret the origin of signal in terms of Albrecht ABC type terms.\cite{Ziegler1988} 
As such, we extend $\beta_{ijk}$ in \autoref{SIVEselection} using the hyper-Raman ABC formalism to understand how vibronic coupling manipulates selection rules for singly and doubly resonant HDFG \autoref{fig:hdfg}. \cite{Ziegler1988, Baranov1990}
...
To employ the Albrecht formalism, we write the states in terms of a Born-Oppenheimer basis $\ket{a,b}$, where $\ket{a,b} = |a(Q)) \otimes \ket{b}$ for electronic states $\{|a(Q))\}$ and vibrational states $\{\ket{b}\}$ (i.e., adiabatic approximation). \cite{BornOppenheimer, Tang1970}
...
The C term is suppressed in the following discussion as it depends on one and two photon forbidden transitions. \cite{Ziegler1988}
Note that other reports obtain $A, B, C$ coefficents through a Herzberg-Teller expansion of the electronic states, where $|a) = |a)_0 + \sum_{a \neq b} \frac{(\partial H / \partial Q )_0  Q}{E_a - E_b} |a)_0$.\cite{HerzbergTeller1933, Neddersen1989, Baranov1990}
This approach is not used here as the expansion of the vibronic dipole moment in terms of normal mode coordinates provides similar insight. 

By compiling all the terms that scale with $\order{Q^0}$, $A$ is written, assuming contraction over the nonresonant vibrational states $\ket{n}$ as
\begin{equation}
	\begin{split}
...
In the case of singly resonant HDFG, $\omega_3$ is also detuned which further contracts the sum over ${v'}$. 
...
Breakdown of the Condon approximation in singly resonant HDFG supports the relative small intensities of HDFG, reported to be roughly an order of magnitude larger than nonresonant background, in hexane and other organic solvents. \cite{RN351, RN353}
...
Other standards include CaF$_2$, a commmonly used optical window used in four wave mixing experiments. \cite{RN362}
...
Orientational averaging shows specific polarization schemes isolate linear combinations of different $\beta_{ijk}$ terms. \cite{Kauranen1996}
...
\end{equation}
Since $\abs{\mu}$ values can be readily extracted from FT-IR spectra,\cite{RN412} HDFG can give quantitative information on the magnitude of $\beta_{ijk}$.
...
\begin{figure}[!htbp]
	...
	\caption{Wave Mixing Energy Level (WMEL) diagrams of interfering HDFG pathways in the  $\vec{k}_4 = -\vec{k}_1 + \vec{k}_2 + \vec{k}_3$ phasematching geometry. 
...
Therefore, to measure coherent dynamics via HDFG, the time ordering shown in \autoref{fig:comparisonwmel} should be used.
