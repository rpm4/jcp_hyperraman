...
By writing $\vec{\mu} = T_{q_1}^{(1)}$ and $\beta_{ijk} = T_{q_1}^{(1)} + T_{q_2}^{(2)} + T_{q_3}^{(3)}$, where $T^{(k)}_{q}$ is a rank k spherical tensor, it is simple to evaluate the integrals in \autoref{sivebeta} with the Wigner-Eckart theorem. \cite{Kowzan2022}
