...
Multiresonant, four wave mixing CMDS experiments, first proposed by Oudar and Shen in 1980,\cite{RN307} directly probe coupling and correlations between different vibrational, electronic, and vibronic states. \cite{RN307, RN281, RN103, Cho2008} 
...
\begin{figure*}[!htbp]
	...
	\caption{Contributions of $A, B$ (\autoref{ABterms_DR}) to DR-HDFG spectrum for a simple two harmonic well system.
		...
		Dotted lines in (b) denote placement of vibronic resonances for $v'$ = 0, 1, 2. }
...
\end{subequations}
\end{widetext}
where N$_{bulk}$ is the bulk number density, and N$_{surf}$ is the surface number density ($\sim$ 10$^{17}$ m$^{-2}$).\cite{RN133, Du1994}	
...
We take the length of layers along the surface normal ($\ell$) to be 10 nm.\cite{RN133}
...
\begin{equation}
	R^{(3)}(t) = \frac{i}{\hbar} \beta_{ijk} \mu_l \exp(-i\Delta_{gv}t) \theta(t) 
\end{equation}  
...
Based upon a simple treatment of orientational averaging, we show that HDFG can provide a probe of the hyper-Raman hyperpolarizability without a need for technically difficult, analytically rigorous hyper-Raman scattering experiments. 
The HDFG method shows promise as a spectroscopic probe of unique electronic structure effects in a variety of molecular systems, but also for understanding the dynamics of single vibrational modes. 
