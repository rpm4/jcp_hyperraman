...
\begin{equation}
	\dot{\rho} = \frac{1}{i \hbar} [H_0 + V(t), \rho] + \dot{\rho}_R,
\end{equation}
where $H$ is system Hamiltonian, $V(t)$ is the field-matter interaction and $\rho_R$ is the relaxation density matrix. 
It is often sufficient to write $\dot{\rho}_R = \Gamma \rho$, i.e. exponential relaxation (Markovian dynamics), where $\Gamma$ is the system's pure dephasing rate. \cite{RN455, RN307}
Before assuming this limit, the system response for an HDFG will be generalized to provide a formalism which supports non-exponential dynamics. %todo: cite Silbey, Mukamel, Kelley etc
