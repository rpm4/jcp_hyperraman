...
For the case of electronic resonance, only a few components of this summation will be relevant ($e,v^\prime$ such that $\Delta_{ev^\prime,g0}$ is small).
...
%todo: note this is the OPPOSITE of hyper raman 
...
The signatures of vibronic coupling in the HDFG spectrum is made clear by using a simple model.
...
\begin{subequations}\label{Hamiltonian}
	...
	\end{equation}
\end{subequations}
for the ground ($H_g$) and first excited ($H_e$) states, where $p$ (momentum) and $q$ (position) are the dimensionless conjugate coordinates of the normal mode, the A and B contributions can be evaluated using Franck-Condon and Herzberg-Teller integrals tabulated in terms of the dimensionless normal mode $q$ and offset parameter $\xi=0.5$ (\autoref{fig:doubres_spec}a).\cite{Carlson1988thesis} % DDK: there is probably a better citation than a thesis.  Carlson wasn't the first to derive these...
For simplicity, we restrict consideration to $v^\prime \in \{0,1,2\}$.
...
% This discussion assumes that mode symmetry allows for coupling between all of the vibronic states considered here.  ddk: there is only one mode!
...
In the pre-resonance region, both $A$ and $B$ terms die off as $\sim \Delta_{e0,g0}^{-1}$.
...
The belief arises from a the approximation $\abs{\omega_{ev',g0} - 2\omega_2} \gg 2\Gamma_{ev',g0}$, which then removes $A$ through a closure relationship.\cite{Neddersen1989}
...
SFG is often consider weak compared to third order spectroscopies because it is usually surface selective,\cite{RN132, RN133} so vSFG detection setups are often quite sensitive.
...
\subsection{Extraction of hyper-Raman cross-sections}

% DDK: make sure statements are correct here
...
Methods reported in the literature to measure absolute hyper-Raman polarizabilities depend upon internal standards such as hyperpolarizabilities of dissolved samples or two-photon absorption cross sections, also difficult to measure. \cite{Okuno2020}  % DDK:  correct reference?
...
% DDK: why does it matter that chi is imaginary?
