...
Based upon these selection rules, we investigate SR-SIVE and DR-SIVE spectroscopy of cyanocobalamin, highlighting the applicability of these methods to non-model systems.
...
The beams are focused into the sample in a BOXCARS geometry.\cite{RN308, Kaufman2024}
...
%maybe talk about maxwell wave eqn for e fields ?
...
where $H_0$ is the system (time independent) Hamiltonian, $\vec{\mu}$ is the electric transition dipole moment, $\vec{E}(\vec{x},t)$ is a time dependent electric field, and the elements of $\bar{\Gamma}$, $\Gamma_{ij}$, are dephasing rates. 
In the rotating frame, $\tilde{\rho}_{ij} = \rho_{ij} e^{-i \omega_{ij}t}$.
By solving \autoref{matrix} under steady state ($\dot{\bar{\rho}}(t) = 0$) in the rotating frame, expressions for $\gamma_{ijkl}$ can be readily obtained by multiplying the output dipole moment, $\mu_i$ by the final density matrix element $\tilde{\rho}_{ij}$. 
...
It is important to note that $\beta_{ijk}$ does not have complete index symmetry.\cite{Andrews1978}
Specifically, it has shown that $T_{q_1}^{(1)}$ fully contributes to $\beta_{ijk}$ but only select $T_{q_2}^{(2)}$ and  $T_{q_3}^{(3)}$ terms contribute. \cite{Andrews1978, Altmann1982, Andrews1990}
%there is some fancy way to note that theres an index constraint but i dont remember the language... i think is fine
...
\begin{figure*}[!htbp]
	...
	\caption{
		...
		$\omega_1$ is stepped in 10 cm$^{-1}$ intervals.
		...
		The spectra are normalized to their most intense feature on $\omega \in [1300, 1700]$ cm$^{-1}$.}
	\label{fig:cobsive}
\end{figure*}
