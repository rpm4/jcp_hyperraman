...
def A(d):
    return Mge * Lge * (f10(d)*f00(d)*Deltaevgo(0,y,0) + f11(d)*f10(d)*Deltaevgo(1,y,1) + f12(d)*f20(d)*Deltaevgo(2,y,2)) #there must be a simpler way to do this but idk
...
def B1(d):
    ...
    # return B10 + B11 + B12
    return Mge*f10(d) * (dLeg * h00(d)) * Deltaevgo(0, y, 0) + Mge*f11(d) * (dLeg* h10(d)) * Deltaevgo(1, y, 1) + Mge*f12(d) * (dLeg * h20(d)) * Deltaevgo(2, y, 2)
...
def B2(d):
    ...
    # return B20 + B21 + B22
    return dMgedQ * Lge *(h10(d) * f00(d)) * Deltaevgo(0, y, 0) + dMgedQ * Lge *(h11(d) * f10(d)) * Deltaevgo(1, y, 1) + dMgedQ * Lge *(h12(d) * f20(d)) * Deltaevgo(2, y, 2) 
